...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=2d0a6ae6d3a44e1f99f6580d5f66516a';
...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="assets/others/icon.jpg"> ');
  }
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      if (app.skippedIntro) return;

      clearCursor();

      writeLine([`<i style='color: #F62459'>access granted</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
